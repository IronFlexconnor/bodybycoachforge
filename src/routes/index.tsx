...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Your daily training plan, AI coach, and progress at a glance.",
      },
    ],
  }),
  component: Home,
...
function Home() {
  ...
  const readiness = checkin
    ? Math.round(
        ((checkin.energy ?? 5) * 7 +
          (10 - (checkin.soreness ?? 5)) * 6 +
          (10 - (checkin.stress ?? 5)) * 4 +
          Math.min(10, checkin.sleep_hours ?? 7) * 3) /
          2,
      )
    : null;
  ...
  const setCount = Array.isArray(todayWorkout?.exercises)
    ? todayWorkout.exercises.reduce((a: number, e: any) => a + (Number(e.sets) || 0), 0)
    : 0;
  ...
  return (
    <AppShell>
      <div className="px-5 pt-12">
        ...
        {!checkin && <CheckinCard onSaved={(c) => setCheckin(c)} />}
        ...
        {todayWorkout ? (
          ...
          >
            ...
            <div className="relative">
              ...
              <p className="mb-5 text-sm text-muted-foreground">
                {exCount} exercises · {setCount} sets · {todayWorkout.focus ?? ""}
              </p>
          ...
          <EmptyProgramCard onBuilt={() => navigate({ to: "/workouts" })} />
        )}
        ...
        <Link
          to="/form"
          className="mb-3 flex items-center gap-4 rounded-2xl border border-primary/30 bg-gradient-card p-4 shadow-card hover:border-primary/60"
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="truncate text-sm text-muted-foreground">
              Video or photo · graded in seconds
            </p>
          </div>
          <ChevronRight className="h-5 w-5 text-muted-foreground" />
        </Link>
        ...
        <Link
          to="/chat"
          className="mb-6 flex items-center gap-4 rounded-2xl border border-border/60 bg-gradient-card p-4 shadow-card hover:border-primary/40"
        >
          <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-gradient-primary text-primary-foreground shadow-glow">
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <Link to="/progress" className="text-sm font-medium text-primary">
            See progress
          </Link>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <Link
            to="/nutrition"
            className="rounded-2xl border border-border/60 bg-gradient-card p-4 shadow-card hover:border-primary/40"
          >
            <div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground">
              <Apple className="h-3.5 w-3.5" /> Nutrition
            </div>
            <div className="text-xl font-bold text-gradient-primary">
              {profile?.macro_targets?.calories ?? "Set"}
            </div>
          ...
          <Card
            icon={TrendingUp}
            title="Adherence"
            value={`${Math.round((stats.weekDone / Math.max(1, stats.weekTotal)) * 100)}%`}
            sub="This week"
          />
        </div>
        ...
        >
          ...
          <div className="relative flex-1 min-w-0">
            ...
            <p className="truncate text-xs text-muted-foreground">
              Tap to see prep videos & swap into your plan
            </p>
          </div>
...
function CheckinCard({ onSaved }: { onSaved: (c: any) => void }) {
  ...
  const save = async () => {
    ...
    supabase.functions
      .invoke("auto-adjust", { body: { trigger: "checkin", auto_apply: true } })
      .then(({ data: a }) => {
        const d = a as any;
        if (d?.should_adjust && d?.summary)
          toast.success(`Plan tuned — ${d.summary}`, { duration: 6000 });
      })
      .catch(() => {});
    setSaving(false);
  };
  ...
  return (
    <div className="mb-5 rounded-3xl border border-primary/20 bg-gradient-card p-5 shadow-card">
      ...
      <p className="mb-3 text-sm text-muted-foreground">
        30 seconds — helps Coach tune today's session.
      </p>
...
function QuickAction({
  ...
  const cls =
    "flex flex-col items-center gap-1.5 rounded-2xl border border-border/60 bg-gradient-card p-2.5 shadow-card hover:border-primary/40 active:scale-95 transition";
  if (to)
    return (
      ...
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="font-bold truncate">
            {currentGoal ? `Currently: ${currentGoal}` : "Pick a goal — Coach builds the program"}
          </div>
        </div>
        <Button
          onClick={() => setOpen((s) => !s)}
          size="sm"
          className="h-9 shrink-0 rounded-full bg-gradient-primary text-xs font-semibold text-primary-foreground shadow-glow"
        >
...
/**
 * Shown when the user has no upcoming sessions (e.g. program generation
 * failed during onboarding, or a block just ended). One tap rebuilds the
 * program right here instead of dead-ending the user.
 */
function EmptyProgramCard({ onBuilt }: { onBuilt: () => void }) {
  const [building, setBuilding] = useState(false);
  const build = async () => {
    setBuilding(true);
    try {
      toast.loading("Coach Forge is designing your program…", { id: "home-gen" });
      const { data, error } = await supabase.functions.invoke("generate-program");
      toast.dismiss("home-gen");
      ...
      toast.dismiss("home-gen");
      toast.error(e instanceof Error ? e.message : "Couldn't build the program — try again.");
    } finally {
      setBuilding(false);
    }
  };
  return (
    <div className="mb-6 rounded-3xl border border-primary/20 bg-gradient-card p-6 text-center shadow-card">
      <div className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-2xl bg-gradient-primary text-primary-foreground shadow-glow">
        <Dumbbell className="h-6 w-6" />
      </div>
      <h2 className="mb-1 text-lg font-bold">Let's build your training plan</h2>
      <p className="mb-4 text-sm text-muted-foreground">
        Coach Forge designs a program around your goal, schedule, and equipment in about 30 seconds.
      </p>
      <Button
        onClick={build}
        disabled={building}
